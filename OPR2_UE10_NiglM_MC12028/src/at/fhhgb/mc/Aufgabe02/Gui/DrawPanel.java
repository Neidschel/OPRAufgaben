package at.fhhgb.mc.Aufgabe02.Gui;

import java.awt.*;
import java.awt.event.*;

import at.fhhgb.mc.Iterable.*;
import at.fhhgb.mc.Observable.MyObserver;
import at.fhhgb.mc.Aufgabe02.List.*;

import at.fhhgb.mc.Aufgabe02.Shapes.*;
import at.fhhgb.mc.Aufgabe02.Shapes.Polygon;
import at.fhhgb.mc.Aufgabe02.Shapes.Rectangle;
 ...
 * 
 * The DrawPanel is now an Observer which recognizes changes in the list and
...
public class DrawPanel extends Panel implements ActionListener,
	...
	public void checkPoints() throws InvalidAccessException {
		...
		if (getButton() == 1 && xpoints.elements() == 2) {
			Rectangle rec = new Rectangle(xpoints.popFront(),
					ypoints.popFront(), xpoints.popFront(), ypoints.popFront());
			xpoints.clear();
			ypoints.clear();
			shapes.pushBack(rec);
			setButton(-1);
		...
		if (getButton() == 4 && xpoints.elements() >= 3) {
			int resultx = Math.abs(xpoints.peekFront() - xpoints.peekBack());
			int resulty = Math.abs(ypoints.peekFront() - ypoints.peekBack());
	 ...
	 */
	@Override
	public void update(GraphicPrimitive _param) {
		repaint();
	}
