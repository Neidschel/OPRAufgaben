...
/**
 * This class represents a binary search tree where numbers can be searched with
 * a binary concept. Following conditions have to be met here: - Every node left
 * of the current node can only contain a smaller number than the one saved in
 * the current node - Every node right of the current node can only contain a
 * higher number than the one saved in the current node - There are no
 * duplicates allowed. (One value can only be once in the tree)
 * 
 * This is not an optimal binary search tree.
 * 
 * Most of the functions are using a recursive algorithm to from the top to the
 * bottom of the tree.
 * 
 * @author Michael Nigl
 * @version 1.0
...
public class BinarySearchTree {
	...
	/**
	 * Helping Method for insert. Determines the location for the new element in
	 * the tree. Duplicate elements are not allowed.
	 * 
	 * @param node
	 *            - variable which is recursive used to go through the tree from
	...
	/**
	 * Searches for the element with the given key. (There can only be one
	...
	/**
	 * Balances the tree deleting the old tree and inserts all values in a new
	 * Tree in the right order for keeping the balance condition.
	 */
	public void balance() {
		Comparable[] tree = toArray(true);
		root = null;
		size = 0;
	...
	/**
	 * A recursive Method which halves the range of the array in every step to
	 * determine the next element for the input.
	 * 
	 * @param low
	 *            the bottom border of the search range for the new value to be
	 *            inserted
	 * @param high
	 *            the top border of the search range for the new value to be
	 *            inserted
	 * @param tree
	 *            a sorted array with all values stored from the previous tree
	 */
		...
		if (root == null) {
			...
			if (root.data.compareTo(key) == 0) {
				...
				if (root.left == null && root.right == null) {
					...
					// inserts the remaining elements which aren't attached to
					// the tree
	 ...
	 *            the top to the bottom; it represents a node of the tree
	 * @param head
	 *            - the node from the previous recursive step. It's needed to
	 *            connect the head to a different node if the key was found in
	 *            the actual node.
	 * @param key
	...
	/**
	 * Another helping method for remove. It inserts the values from node and
	 * all nodes which are hierarchically below it once more into the tree. (the
	 * root tree)
	 * 
	 * @param node
	 *            - variable which is recursive used to go through the tree from
	 ...
	 * 
	 * @param ascending
	 *            - true if its ordered ascending; false if its ordered
	 *            descending
	...
	/**
	 * Helping Method for toArray. Determines the elements in the tree saves on
	 * element after another into the array.
	 * 
	 * @param elements
	 ...
	 *            descending
	 * @param offset
	 *            - a helping variable which is used to determine the index
	 *            where to save the next value into the array
	 * @param node
	 *            - variable which is recursive used to go through the tree from
	 *            the top to the bottom; it represents a node of the tree
	 * @return offset - returns the index for the next insert
	 */
	private int toArray(Comparable[] elements, boolean ascending, int offset,
			BinaryTreeNode node) {
	...
	/**
	 * Helping Method for max. Determines which is the element with the highest
	 * value in the tree.
	 * 
	 * @param node
	 *            - variable which is recursive used to go through the tree from
	...
	/**
	 * Helping Method for min. Determines which is the element with the lowest
	 * value in the tree.
	 * 
	 * @param node
	 *            - variable which is recursive used to go through the tree from
	...
	/**
	 * Helping Method for String. Determines which element is printed on which
	 ...
	 *            the top to the bottom; it represents a node of the tree
	 * @param tree
	 *            - a multidimensional Array where the positions for the tree
	 *            elements are saved
	 * @param position
	 *            - saves the index for the recursive depth and the index for
	 *            the next element to write into "tree"
	 * @param spaces
	 *            - used to center the numbers if it hasn't the length of the
	 *            highest number in the tree
	 * @return position - returns the index for the next insert
	 */
	private int[] toString(BinaryTreeNode node, String[][] tree,
			int[] position, int spaces) {
